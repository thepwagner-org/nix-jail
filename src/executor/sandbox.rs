...
impl Executor for SandboxExecutor {
    async fn execute(&self, config: ExecutionConfig) -> Result<ExecutionHandle, ExecutorError> {
        ...
        let (final_command, env) = if !config.store_paths.is_empty() {
            ...
            // Canonicalize job_dir for home/bin paths
            let canonical_job_dir = config
                .job_dir
                .canonicalize()
                .unwrap_or_else(|_| config.job_dir.clone());
        ...
        let io_handle = if config.interactive {
            ...
            drop(tokio::spawn(async move {
                ...
                let exit_code = match tokio::time::timeout(timeout, wait_task).await {
